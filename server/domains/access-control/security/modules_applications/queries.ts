...
export const getModuleApplicationById = cache((moduleId: string | number) => 
  moduleApplicationsRepository.getById(moduleId)
);
...
export const getModuleProfile = cache(async (moduleId: string | number) => {
  const [module, applications, stats, activeApplications, configs] = await Promise.all([
...
//   const [modules, allStats] = await Promise.all([
...
export const getModulesByCategory = cache(async (category: string) => {
  ...
  const filteredModules = modules.content.filter((module: IModuleApplication) => module.category === category);
...
export const getApplicationsByModuleWithDetails = cache(async (moduleId: string | number) => {
  ...
  const applicationsWithDetails = applications.map(application => {
    const relationship = moduleApplications.content.find((ma: IModuleApplication) => ma.application_id === application.id_application);
    
    return {
      ...application,
      relationship: {
        is_active: relationship?.deleted,
