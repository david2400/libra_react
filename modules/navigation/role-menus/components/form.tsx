...
import {
  IRoleMenuCreateRequest,
  IRoleMenuUpdateRequest,
} from "../models/role-menu.interface";
import Swal from "sweetalert2";
import { useRouter } from "next/navigation";
import {
  createRoleMenuServerAction,
  updateRoleMenuServerAction,
} from "@/app/[locale]/(protected)/navigation/role-menus/actions";
...
export const RegisterRoleMenu = ({}: IFormAddProps = {}) => {
  ...
  const handleSubmit: SubmitHandler<IRoleMenuCreateRequest> = async (
    values,
  ) => {
    try {
      const result = await createRoleMenuServerAction(
        values.roleId,
        values.menuId,
        values,
      );
  ...
  return (
    <FormBase
      ...
      validationSchema={validationRoleMenu()}
    />
  );
};
...
export const UpdateRoleMenu = ({
  initialValues,
}: IFormUpdateProps<IRoleMenuUpdateRequest>) => {
  const router = useRouter();
  const { useTranslations } = require("next-intl");
  const t = useTranslations("navigation.roleMenus.messages");
  const tMessages = useTranslations("messages");

  const handleSubmit: SubmitHandler<IRoleMenuUpdateRequest> = async (
    values,
  ) => {
    if (!values.roleId || !values.menuId) return;

    try {
      const result = await updateRoleMenuServerAction(
        values.roleId,
        values.menuId,
        values,
      );
